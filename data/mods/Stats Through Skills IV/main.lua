...
function calc_bonus(skills_set)
    ...
    for _,s in ipairs(skills_set) do
		skill_level = player:get_skill_level(skill_id(s))
		if skill_level > 1 then skill_total = skill_total + skill_level end
    end
	
	local bonus_amount = math.floor(math.sqrt(skill_total))
    if skill_total < 5 then
       bonus_amount = 1
	end
    if skill_total < 2 then
        bonus_amount = 0
    end
